...
# ADR-001: Fluxo de Registro Atômico de GoatFarm
...
4.  **Camada Arquitetural:** O fluxo de criação atômica é implementado na **Application/Business Layer**, não sendo responsabilidade direta das entidades JPA.
5.  **Transação & Rollback:** Qualquer falha durante a criação de User, Farm, Address ou Phones resulta em **rollback completo** da transação.
6.  **Cobertura de Testes:** A decisão é protegida por testes unitários (`GoatFarmBusinessTest`), garantindo que o contrato não seja violado em refatorações futuras.
