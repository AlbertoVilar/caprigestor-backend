-- A) Diagnóstico: localizar pregnancies duplicadas com status ACTIVE por (farm_id, goat_id)
...
-- B) Fix seguro: mantém a ACTIVE mais recente e fecha as demais
UPDATE pregnancy p 
SET status = 'CLOSED',
...
-- C) Verificação final: confirmar que não há mais duplicidades ACTIVE por (farm_id, goat_id)
SELECT farm_id, goat_id, COUNT(*) AS active_count
FROM pregnancy
WHERE status = 'ACTIVE'
GROUP BY farm_id, goat_id
